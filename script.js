...
// Control de carga perezosa por pestaña
const cargadoUnaVez = {};
...
// ============================================
// MODALES: CERRAR CON CLIC EN FONDO O ESCAPE
// ============================================
function initModales() {
  const modal = document.getElementById('modal');
  const selector = document.getElementById('selectorListasModal');
  ...
  if (selector) {
    selector.addEventListener('click', (e) => {
      if (e.target === selector) cerrarSelectorListas();
    });
  }
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (selector && selector.style.display !== 'none') { cerrarSelectorListas(); return; }
    const recs = document.getElementById('modalRecordatorios');
    if (recs && recs.style.display !== 'none') cerrarModalRecordatorios();
  });
}
...
function registrarPWA() {
  ...
  window.addEventListener('appinstalled', () => {
    ...
    mostrarNotificacion('✅ App instalada', 'success');
  });
}
...
window.addEventListener('scroll', () => {
  ...
  if (id === 'explorar') {
    if (subExplorarActual === 'peliculas' && peliculasPage <= 10) {
      cargarPeliculas(false);
    } else if (subExplorarActual === 'series' && seriesPage <= 10) {
      cargarSeries(false);
    } else if (subExplorarActual === 'tendencias' && tendenciasPage <= 10) {
      cargarTendencias(false);
    }
  } else if (id === 'buscar' && currentSearch && busquedaPage <= 10) {
    buscar(true);
  } else if (id === 'estrenos' && !agendaCargando) {
...
// ============================================
// Mapeo de secciones antiguas a la nueva estructura con pestañas
...
let subExplorarActual = 'tendencias';
let vistaEstrenosActual = 'agenda';

function cambiarSubExplorar(sub) {
  subExplorarActual = sub;
  document.querySelectorAll('#explorar .subtab').forEach(b => b.classList.toggle('active', b.dataset.sub === sub));
  document.querySelectorAll('#explorar .vista-sub').forEach(v => v.classList.toggle('activa', v.id === `vista-${sub}`));
  
  // Carga perezosa: solo la primera vez que se visita cada pestaña
  if (!perfilCompartido) {
    const clave = `exp_${sub}`;
    if (!cargadoUnaVez[clave]) {
      cargadoUnaVez[clave] = true;
      if (sub === 'tendencias') cargarTendencias(true);
      else if (sub === 'peliculas') cargarPeliculas(true);
      else if (sub === 'series') cargarSeries(true);
    }
  }
}

function cambiarVistaEstrenos(vista) {
  vistaEstrenosActual = vista;
  document.querySelectorAll('#estrenos .subtab').forEach(b => b.classList.toggle('active', b.dataset.sub === vista));
  document.querySelectorAll('#estrenos .vista-sub').forEach(v => v.classList.toggle('activa', v.id === `vista-${vista}`));
}
...
function mostrarSeccion(id) {
  ...
  // Compatibilidad: las antiguas secciones ahora son pestañas internas
  if (MAPA_SECCIONES[id] && id !== idReal) {
    idReal = MAPA_SECCIONES[id];
  }
  
  if (['tendencias', 'peliculas', 'series'].includes(id)) {
    subExplorarActual = id;
    document.querySelectorAll('#explorar .subtab').forEach(b => b.classList.toggle('active', b.dataset.sub === id));
    document.querySelectorAll('#explorar .vista-sub').forEach(v => v.classList.toggle('activa', v.id === `vista-${id}`));
  }
  if (id === 'calendario' || id === 'agenda') {
    cambiarVistaEstrenos(id);
  }
  
  document.querySelectorAll('.seccion').forEach(s => s.style.display = 'none');
  const target = document.getElementById(idReal);
  if (!target) return;
  
  target.style.display = 'block';
  window.scrollTo(0, 0);
  marcarNavActiva(idReal);
  
  if (perfilCompartido) return;
  
  switch(idReal) {
    case 'explorar':
      if (!cargadoUnaVez[`exp_${subExplorarActual}`]) {
        cargadoUnaVez[`exp_${subExplorarActual}`] = true;
        if (subExplorarActual === 'tendencias') cargarTendencias(true);
        else if (subExplorarActual === 'peliculas') cargarPeliculas(true);
        else if (subExplorarActual === 'series') cargarSeries(true);
      }
      break;
    case 'estrenos':
      if (vistaEstrenosActual === 'agenda') cargarAgenda(true);
      break;
    case 'miLista':
      renderListas();
      break;
...
// ============================================
function cambiarFiltroPeliculas(filtro, btn) {
  if (perfilCompartido) return;
  filtroPeliculas = filtro;
  document.querySelectorAll('#vista-peliculas .seg-btn').forEach(b => b.classList.remove('active'));
...
// ============================================
function cambiarFiltroSeries(filtro, btn) {
  if (perfilCompartido) return;
  filtroSeries = filtro;
  document.querySelectorAll('#vista-series .seg-btn').forEach(b => b.classList.remove('active'));
  if (btn) btn.classList.add('active');
  seriesPage = 1;
  cargarSeries(true);
}
...
// ============================================
function cambiarTipoTendencias(tipo, btn) {
  if (perfilCompartido) return;
  tendenciasTipo = tipo;
  document.querySelectorAll('#vista-tendencias .seg-btn').forEach(b => b.classList.remove('active'));
  if (btn) btn.classList.add('active');
  tendenciasPage = 1;
...
// ============================================
// Cache en memoria para evitar N llamadas repetidas al hacer scroll/buscar/cambiar sección
const _providersCache = new Map();

async function enriquecerConPlataformas(item, tipo) {
  // --- Plataformas (cacheado por id para no repetir llamadas) ---
  const cacheKey = `${tipo}-${item.id}`;
  let plataformas = _providersCache.get(cacheKey);
  ...
  if (!plataformas) {
    try {
      ...
      plataformas = (data.results?.ES?.flatrate || []).map(p => ({
        ...p,
    ...
    _providersCache.set(cacheKey, plataformas);
  }
  
  item.plataformas = plataformas;
  
  // --- Campos de serie: ya vienen en los listados de TMDB, no hacemos fetch extra ---
  if (tipo === 'tv') {
    item.next_episode_to_air = item.next_episode_to_air || null;
    item.last_episode_to_air = item.last_episode_to_air || null;
...
function abrirModal(item) {
  ...
  if (botonVisto) {
    botonVisto.textContent = estaVisto ? '↩️ Quitar visto' : '✅ Visto';
    botonVisto.style.background = estaVisto
...
// ============================================
// SINCRONIZAR BADGE "VISTO" EN TARJETAS
// ============================================
function sincronizarBadgesVisto(itemId, visto) {
  document.querySelectorAll(`.card[data-id="${itemId}"]`).forEach(card => {
    let badge = card.querySelector('.visto-badge');
    if (visto && !badge) {
...
function marcarComoVisto() {
  ...
  if (index === -1) {
    ...
    if (boton) {
      boton.textContent = '↩️ Quitar visto';
      boton.style.background = 'linear-gradient(135deg, #f39c12, #d68910)';
    }
    sincronizarBadgesVisto(itemActual.id, true);
  } else {
    ...
    if (boton) {
      ...
      boton.style.background = 'linear-gradient(135deg, var(--success), var(--success-dark))';
    }
...
function mostrarSelectorListas() {
  ...
  document.getElementById('selectorListasModal').style.display = 'flex';
}
